@extends('layouts.app')

@section('content')
<div class="container">
    <h3>Edit Kamar</h3>
    <form action="{{ route('kamar.update', $kamar->id) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="mb-3">
            <label>ID</label>
            <input type="text" name="id" class="form-control" value="{{ $kamar->id }}" readonly>
        </div>
        <div class="mb-3">
            <label>Lantai</label>
            <input type="number" name="lantai" class="form-control" value="{{ $kamar->lantai }}" required>
        </div>
        <div class="mb-3">
            <label>Kapasitas</label>
            <input type="text" name="kapasitas" class="form-control" value="{{ $kamar->kapasitas }}" required>
        </div>
        <div class="mb-3">
            <label>Fasilitas</label>
            <input type="text" name="fasilitas" class="form-control" value="{{ $kamar->fasilitas }}" required>
        </div>
        <div class="mb-3">
            <label>Tarif</label>
            <input type="number" name="tarif" class="form-control" value="{{ $kamar->tarif }}" required>
        </div>
        <button type="submit" class="btn btn-primary">Update</button>
        <a href="{{ route('kamar.index') }}" class="btn btn-secondary">Batal</a>
    </form>
</div>
@endsection
